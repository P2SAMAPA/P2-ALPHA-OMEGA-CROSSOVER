...
Loads data → computes Alpha/Omega crossovers for all tickers →
applies EVT overlay → builds JSON outputs → uploads to HuggingFace.
"""
    ...
    for universe_name, tickers in config.UNIVERSES.items():
        ...
        for ticker in available:
            ...
            if result.get("action") != "INSUFFICIENT DATA":
                ...
                ticker_tail_index[ticker] = result.get("tail_index", 0)
        ...
        if ticker_signals:
            ...
            for ticker, z in z_scores.items():
                if z > config.CROSSOVER_THRESHOLD:
                    actions[ticker] = "STRONG BUY"
                elif z > 0.2:
                    actions[ticker] = "BUY"
                elif z > -0.2:
                    actions[ticker] = "HOLD"
                elif z > -config.CROSSOVER_THRESHOLD:
            ...
            results_tab1["universes"][universe_name] = {
                ...
                "full_scores": {
                    t: {
                        ...
                        "tail_index": ticker_tail_index.get(t, 0),
